...
list($clav24, $valor24) = each($_POST['mueble']);
...
list($clav25, $valor25) = each($_POST['fuga']);
...
list($clav26, $valor26) = each($_POST['falta']);
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form2")) {
    ...
    $insertSQL = sprintf("INSERT INTO nomina_vent (fecha_na, nombre, sueldo, extra, total_ing, seguro, vales, prestamo, des_pres, moto, desc_moto, ot_conc, ot_impo, total_egr, total, trabajadores_idtrabajadores,muebles,fugas,faltantes,devoluciones,n_ventas,enganches,ventas,comision,sal_mueb,sal_fuga,sal_falta) VALUES (%s,%s,%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s,%s,%s,%s,%s,%s,%s)",
...
$query_Recordset11 = "SELECT * FROM penaliza WHERE fecha_p BETWEEN  '".$f."' AND '" .$f1."'";
...
<script>
    ...
    var sueldo = (parseFloat(por)/100)*parseFloat(cobro);
    ...
    var tingre = parseFloat(extra) + parseFloat(sueldo);
    ...
    var total = parseFloat(tingre) - parseFloat(tegres);
    ...
    var ttotal = document.getElementById("ttotal").value;
     ...
     //ttotal = parseFloat(ttotal) + parseFloat(total);
    ...
    ttotal =0;
     ...
     for(var x=1; x<=<?php echo $totalRows_Recordset1;?>;x++){
        ...
        var ttotal = parseFloat(tt) + parseFloat(ttotal);
...
?> 
        ...
        <p class="enca">N¢mina de Sueldos
...
<a href="./nom_v.php?sucursal=<?php echo $sucursal;?>" target="_new" >N¢mina Imprimible</a>
...
<table border="1" align="center" rules="all">
    ...
    <td rowspan="2" align="center" valign="middle"><strong>Nø Ventas</strong></td>
    ...
    <td rowspan="2" align="center" valign="middle"><p><strong>Comisi¢n %</strong></p></td>
    ...
    <td rowspan="2" align="center" valign="middle"><strong>Penalizaci¢n</strong></td>
...
$sue = $row_Recordset1['SUM(cantidad)'] * 0.05; echo $sue;?>" size="5" readonly="readonly"/></td>
     ...
     if($row_Recordset1['idtrabajadores'] == $row_Recordset9['trabajadores_idtrabajadores']){
                        ...
                        if((! in_array($row_Recordset9['trabajadores_idtrabajadores'], $losid))){
...
//************************************************************************************************** ?> <input name="seguro[]" type="hidden" id="seguro" value="<?php echo $seguro;?>" size="3" /> 
     ...
     if($row_Recordset1['idtrabajadores'] == $row_Recordset3['trabajadores_idtrabajadores']){
                     ...
                     if(! in_array($row_Recordset3['trabajadores_idtrabajadores'], $losid)){
                                ...
                                $d_presta1 = $row_Recordset3['adeudo'];
...
do{
     ...
     if($row_Recordset1['idtrabajadores'] == $row_Recordset3['trabajadores_idtrabajadores']){
                     ...
                     $presta1=$row_Recordset3['cob_sem'];
...
//************************************************************************************************** ?>
 ...
 $otr=""; do{
     ...
     if($row_Recordset1['idtrabajadores'] == $row_Recordset10['trabajadores_idtrabajadores']){
                        ...
                        if((! in_array($row_Recordset10['trabajadores_idtrabajadores'], $losid))&&($row_Recordset10['concepto']!="Mueble")&&($row_Recordset10['concepto']!="Faltantes")&&($row_Recordset10['concepto']!="Fugas")){
...
<input type="hidden" name="ot_conc[]" value="<?php echo $otr; ?>" size="5" />
...
//************************************************************************************************** ?>
     ...
     <input type="hidden" name="ot_impo[]" value="<?php echo $otr2; ?>" size="5" />
...
//************************************************************************************************** ?>
     ...
     <?php $Recordset11 = mysql_query($query_Recordset11, $conexion) or die(mysql_error());?>
...
//************************************************************************************************** ?>
     ...
     $tegres = $seguro + $moto + $presta + $vale + $ex_ded + $mueble + $fuga + $falta + $pena+$otr2;
...
//."->".$ex_ded."+".$otr."+".$mueble."+".$fuga."+".$pena."+".$falta."+".$presta."+".$moto."+".$seguro."+".$vale."+".
...
<?php /*do{
 ...
 <input type="password" name="contra" id="contra" onkeyup="validar()" />

     <td><input type="submit" value="Guardar N¢mina" disabled="disabled" id= "bton" /></td>
